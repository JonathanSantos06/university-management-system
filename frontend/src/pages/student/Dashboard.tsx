...
export const StudentDashboard: React.FC = () => {
  ...
  const inProgress = mockCurriculum.filter(
    (s) => s.status === "CURSANDO",
  ).length;
    ...
    <StudentLayout title="Dashboard">
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {mockStudentProfile.career}
            {" · "}
            Semestre {mockStudentProfile.currentSemester}
            {" · "}
            Matrícula {mockStudentProfile.studentCode}
          </Typography>
        </Box>
      </Box>
      ...
      <Grid container spacing={2} mb={3}>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard label="Aprobadas" value={approved} color="green" />
        </Grid>

        <Grid item xs={12} sm={6} md={3}>
          <StatCard label="Reprobadas" value={failed} color="#C0392B" />
        </Grid>

        <Grid item xs={12} sm={6} md={3}>
          <StatCard label="Cursando" value={inProgress} color="#0B3D91" />
        </Grid>

        <Grid item xs={12} sm={6} md={3}>
          <StatCard label="Pendientes" value={pending} color="#8A8F99" />
        </Grid>
      </Grid>
